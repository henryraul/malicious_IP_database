...
class MetodoHTTPTestCase(TestSetUp):
    
    REPETICIONES = 20 # Son 9 los métodos HTTP standares
    
    url = '/peticion/metodoHTTP/'
    
    def crear_metodo_HTTP_JSON(self):
        return {
            'metodo_HTTP': faker.http_method()
        }
    
    def test_metodo_HTTP_listcreate(self):
        for i in range(self.REPETICIONES):
        
            #Mandamos a crear un nuevo nombre de método HTTP y devolverlo en formato JSON
            metodo_http_prueba_JSON = self.crear_metodo_HTTP_JSON()
